...
# Trimming the data's dimensionality
...
## Discarding columns with inadequate data
...
```{r}
pmlColumnFillCounts <- sapply(pmlTraining, function(col) { sum(!(is.na(col) | col=="")) })
pmlFullColumns <- names(pmlColumnFillCounts[pmlColumnFillCounts == length(pmlTraining$classe)])
```

## Discarding columns to prevent overfitting
...
```{r}
pmlFullColumns <- pmlFullColumns[!pmlFullColumns %in% c("X", "user_name", "raw_timestamp_part_1", "raw_timestamp_part_2", "cvtd_timestamp", "new_window", "num_window", "classe")]
```
...
pmlTuneGrid <- data.frame(interaction.depth = 4, n.trees = 300, shrinkage = .1)

pmlModel <- train(pmlTrainingTrimmed, 
                  pmlTraining$classe, 
                  method="gbm", 
                  trControl=pmlTrainControl,
...
We can thus conclude that the out-of-sample accuracy will be 
`r pmlConfusionMatrix$overall['Accuracy']`. 
...
```{r}
1 - pmlConfusionMatrix$overall['Accuracy']
```

# Determining the answers for the unlabeled data
